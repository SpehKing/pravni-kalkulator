<template>
  <div class="card-float p-5">
    ...
    >
      ...
      >
        <svg width="13" height="13" viewBox="0 0 16 16" fill="white">
          <path
            d="M8 1L1 14h14L8 1zm0 2.5l5.5 9.5H2.5L8 3.5zM7.25 7v3h1.5V7h-1.5zm0 4v1.5h1.5V11h-1.5z"
          />
        </svg>
      </div>
      <h3
    ...
    <div
      v-if="aml.redFlags && aml.redFlags.length > 0"
    ...
    >
      ...
      <ul style="display: flex; flex-direction: column; gap: 0.35rem">
        ...
        >
          <span
            :style="flagDotStyle(flag.severity)"
            style="
              width: 6px;
              height: 6px;
              border-radius: 50%;
    ...
    <!-- PEP Assessment -->
    <div
      v-if="aml.pepAssessment && aml.pepAssessment.flagged"
      style="
        margin-top: 0.75rem;
        background: #fff0ee;
    ...
    >
      ...
      <ul style="display: flex; flex-direction: column; gap: 0.3rem">
        <li
          v-for="(pep, i) in aml.pepAssessment.persons"
          :key="i"
          style="
            display: flex;
            align-items: flex-start;
        ...
        >
          <span
            ...
            "
          ></span>
          <span style="color: #2e2a26"
            ><strong>{{ pep.name }}</strong> — {{ pep.reason }}</span
          >
        </li>
      </ul>
      <div style="margin-top: 0.5rem; font-size: 0.7rem; color: #a04040">
        Pred nadaljevanjem je obvezna okrepljena ZPPDFT-2 skrbnost za PEP osebe.
      </div>
    </div>
  </div>
</template>
