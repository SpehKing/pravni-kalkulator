...
const HIGH_RISK_JURISDICTIONS = [
  ...
  'Seychelles', 'Belize', 'Vanuatu', 'Marshall Islands', 'Bahamas'
]
const AML_KEYWORDS_HIGH = [
  'cash contribution', 'cash payment', 'cash capitalization',
  'time is of the essence', 'as quickly as possible', 'within 2 weeks', 'very quickly',
  'holding company', 'nominee', 'trust structure', 'beneficial owner'
]
const AML_KEYWORDS_MEDIUM = [
  'urgent', 'immediately', 'quickly', 'IP assets', 'IP transfer',
  'recently incorporated', 'newly formed', 'no questions'
]

function normalizeEntityName(name) {
  return name.toLowerCase()
    .replace(/\b(d\.o\.o\.|d\.d\.|a\.g\.|gmbh|ltd|llc|inc|s\.r\.o\.|s\.a\.)\b/gi, '')
    .replace(/[.,]/g, '')
    .trim()
}
...
function checkConflict(entities) {
  ...
  for (const entity of entities) {
    ...
    for (const client of clients) {
      ...
      if (normalizedEntity.length > 3 && (
        normalizedClient.includes(normalizedEntity) || normalizedEntity.includes(normalizedClient)
      )) {
        matches.push({
      ...
      for (const cp of (client.counterparties || [])) {
        ...
        if (normalizedEntity.length > 3 && (
          normalizedCp.includes(normalizedEntity) || normalizedEntity.includes(normalizedCp)
        )) {
          matches.push({
            extractedEntity: entity.name,
            matchedClient: cp,
  ...
  if (matches.length === 0) return { status: 'NO_CONFLICT', matches: [], blockEmail: false }
...
function runAmlPreScreening(emailText) {
  ...
  for (const j of HIGH_RISK_JURISDICTIONS) {
    if (text.includes(j.toLowerCase())) {
      flags.push({ flag: `High-risk jurisdiction detected: ${j}`, severity: 'high', source: 'Jurisdiction screening' })
      ruleBasedRiskScore += 3
    }
  }
  for (const kw of AML_KEYWORDS_HIGH) {
    if (text.includes(kw.toLowerCase())) {
      flags.push({ flag: `AML indicator: "${kw}"`, severity: 'high', source: 'Keyword screening' })
      ruleBasedRiskScore += 2
    }
  }
  for (const kw of AML_KEYWORDS_MEDIUM) {
    if (text.includes(kw.toLowerCase())) {
      flags.push({ flag: `AML indicator: "${kw}"`, severity: 'medium', source: 'Keyword screening' })
      ruleBasedRiskScore += 1
    }
  }
  const cashMatch = emailText.match(/EUR\s*([\d,]+(?:\.\d+)?)\s*(?:million)?/gi)
  if (cashMatch) {
    for (const match of cashMatch) {
      const numStr = match.replace(/[^0-9.,]/g, '').replace(',', '')
      const amount = parseFloat(numStr)
      if (!isNaN(amount) && amount >= 15000) {
        flags.push({ flag: `Large cash transaction: ${match}`, severity: 'high', source: 'ZPPDFT-2 cash threshold' })
        ruleBasedRiskScore += 3
      }
    }
  }
...
- keyFacts: 3-6 bullet points`
  ...
  return {
    language: ['sl', 'en', 'mixed'].includes(result.language) ? result.language : 'en',
    legalDomains: Array.isArray(result.legalDomains) ? result.legalDomains.slice(0, 10) : [],
    entities: Array.isArray(result.entities) ? result.entities.slice(0, 20).map(e => ({
      name: String(e.name || ''), type: e.type || 'company', role: String(e.role || '')
    })) : [],
    urgency: ['normal', 'high', 'critical'].includes(result.urgency) ? result.urgency : 'normal',
    deadlines: Array.isArray(result.deadlines) ? result.deadlines.slice(0, 5).map(d => ({
      description: String(d.description || ''), deadlineType: d.deadlineType || 'statutory',
      hoursRemaining: d.hoursRemaining != null ? Number(d.hoursRemaining) : null, source: String(d.source || '')
    })) : [],
    summary: String(result.summary || ''),
    keyFacts: Array.isArray(result.keyFacts) ? result.keyFacts.slice(0, 8).map(String) : [],
  }
}
...
- If urgency is critical, reflect urgency in draft tone`
  ...
  return {
    assignedTeam: Array.isArray(result.assignedTeam) ? result.assignedTeam.slice(0, 3).map(l => ({
      lawyerId: String(l.lawyerId || ''), name: String(l.name || ''), title: String(l.title || ''),
      matchReason: String(l.matchReason || ''), relevanceScore: Math.min(100, Math.max(0, Number(l.relevanceScore) || 0))
    })) : [],
    ...
    feeEstimate: {
      matterType: String(result.feeEstimate?.matterType || ''), hoursRange: String(result.feeEstimate?.hoursRange || ''),
      feeRange: String(result.feeEstimate?.feeRange || ''), currency: 'EUR'
    },
    matterForm: {
      clientName: String(result.matterForm?.clientName || ''), matterDescription: String(result.matterForm?.matterDescription || ''),
      practiceArea: String(result.matterForm?.practiceArea || ''), responsibleLawyer: String(result.matterForm?.responsibleLawyer || ''),
      estimatedValue: String(result.matterForm?.estimatedValue || '')
    },
    draftEmail: {
      subject: String(result.draftEmail?.subject || ''), body: String(result.draftEmail?.body || ''),
...
async function checkCompliance(emailText, entities) {
  ...
  const { flags: preScreenFlags, ruleBasedRiskScore } = runAmlPreScreening(emailText)
...
- Do NOT duplicate pre-screening flags verbatim — synthesize and add context`
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: `Analyze this email for AML/KYC compliance:\n\n${emailText}` }
    ],
    max_completion_tokens: 2000,
  })
  const amlResult = JSON.parse(completion.choices[0].message.content)

  const allRedFlags = [...preScreenFlags, ...(Array.isArray(amlResult.redFlags) ? amlResult.redFlags : [])]
  ...
  return {
    conflict: { status: conflict.status, matches: conflict.matches, blockEmail: conflict.blockEmail },
    aml: {
      riskLevel: ['STANDARD', 'ENHANCED', 'REFUSE'].includes(amlResult.riskLevel)
        ? amlResult.riskLevel
        : ruleBasedRiskScore >= 11 ? 'REFUSE' : ruleBasedRiskScore >= 6 ? 'ENHANCED' : 'STANDARD',
      redFlags: uniqueFlags.slice(0, 12),
      kycDocuments: Array.isArray(amlResult.kycDocuments) ? amlResult.kycDocuments.slice(0, 10).map(String) : [],
...
async function processEmail(id, emailText) {
  ...
  console.log(`  [${id}] Step 2: generate-draft + check-compliance (parallel)...`)
